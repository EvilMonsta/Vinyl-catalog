...
class UserServiceTest {
    ...
    @Test
    void getUser_shouldCacheAndReturnIfNotInCache() {
        User user = new User();
        user.setId(2);

        when(userCache.contains("user-2")).thenReturn(false);
        when(userRepository.findById(2)).thenReturn(Optional.of(user));

        User result = userService.getUser(2);
        assertThat(result.getId()).isEqualTo(2);

        verify(userCache).put("user-2", user);
    }
    ...
    @Test
    void createUser_shouldFailIfUsernameExists() {
        UserDto dto = new UserDto();
        dto.setEmail("notused@mail.com");
        dto.setUsername("used");
        ...
        assertThatThrownBy(() -> userService.createUser(dto))
                .isInstanceOf(ResponseStatusException.class)
                .hasMessageContaining("username уже существует");
    }
    ...
    @Test
    void hashPassword_shouldThrowIfAlgorithmInvalid() {
        try (MockedStatic<MessageDigest> mdMock = mockStatic(MessageDigest.class)) {
            ...
            assertThatThrownBy(() -> userService.hashPassword("123"))
                    .isInstanceOf(IllegalStateException.class)
                    .hasMessageContaining("hashing the password");
        }
    }
    ...
    void updateUserRole_shouldThrowIfUserNotFound() {
        ...
        assertThatThrownBy(() -> userService.updateUserRole(999, 1))
                .isInstanceOf(ResponseStatusException.class)
    ...
    void updateUserRole_shouldUpdateSuccessfully() {
        ...
        User user = new User();
        user.setId(userId);
        user.setUsername("rolechanger");

        Role newRole = new Role();
        when(userRepository.findById(userId)).thenReturn(Optional.of(user));
        when(roleService.getRoleById(roleId)).thenReturn(newRole);
        when(userRepository.save(any(User.class))).thenAnswer(inv -> inv.getArgument(0));
        when(userRepository.findAll()).thenReturn(Collections.emptyList());

        User result = userService.updateUserRole(userId, roleId);

        assertThat(result.getRole()).isEqualTo(newRole);
        verify(userCache).put("user-1", result);
        verify(userListCache).put(eq("all-users"), any());
        verify(userByUsernameCache).put("user-username-rolechanger", List.of(result));
    }
